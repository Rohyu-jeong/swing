...
-- SQL 문장을 기능과 성능적인 측면에서 효율적으로 작성할 수 있기 때문에, 모델링 작업은 데이터베이스 설계의 핵심 과정이다.
...
-- 1. 요구사항 분석
-- 고객과의 의사소통을 통해 고객의 업무 프로세스를 완벽하게 이해하고 그들의 요구를 구체적으로 뽑아내는 과정
...
-- 정규화는 데이터 모델의 일관성을 확보하고 중복을 제거하여 신뢰성 있는 데이터 구조를 얻는데 목정이 있다.
...
/*
 * 1. 요구사항
 * 꽃 테이블이랑 화분테이블 두 개가 필요하고 꽃을 구매할 때 화분도 같이 구매한다.
 * - 꽃 테이블에는 꽃 이름, 색깔, 가격이 있다.
 * - 화분 테이블에는 제품번호, 화분 색깔, 화분 모양, 꽃 이름이 있다.
 * 
 * 꽃
 * */
