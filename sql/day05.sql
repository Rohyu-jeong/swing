-- HAVING절
-- GROUP BY로 집계된 값 중 WHERE절처럼 특정 조건을 추가하는 것
-- 그룹 함수에 조건을 달아야 할 때

-- 각 부서별 급여의 최대값, 최소값, 인원 수를 출력하되
-- 급여의 최대값이 8000이상인 결과만 보여줄 것
...
GPOUP BY DEPARTMENT_ID, JOB_ID;
-- HAVING에 해도 결과는 나오는데, 일반 컬럼은 WHERE에 하는게 속도가 조금 더 빠르다.
...
-- 여러번 DB 접속이 필요한 상황을 한 번으로 줄여서 속도를 중가시킬 수 있다.
-- 서브쿼리를 사용할 수 있는 곳
-- WHERE, HAVING
-- SELECT, DELETE 문의 FROM 절
-- UPDATE문의 SET절
-- INSERT문의 INTO절
...
-- 사원테이블에서 이름이 'Michael'이고, 직종이 'MK_MAN'인 사원의 급여
SELECT SALARY
FROM EMPLOYEES
WHERE FIRST_NAME = 'Michael' AND JOB_ID = 'MK_MAN'; -- 13,000
...
-- 많이 받는 사원의 정보를 사번, 이름, 직종, 급여 순으로 출력
SELECT EMPLOYEE_ID, FIRST_NAME, JOB_ID, SARALY
FROM EMPLOYEES
WHERE JOB_ID = (SELECT JOB_ID FROM EMPLOYEES WHERE EMPLOYEE_ID = 111) 
OR SALARY > (SELECT SALARY FROM EMPLOYEES WHERE EMPLOYEE_ID = 159);

-- 100번 부서의 최소 급여보다 많이 받는 다른 부서의 부서번호, 최소 급여를 조회
SELECT DEPARTMENT_ID, MIN(SARALY)
FROM EMPLOYEE
GROUP BY DEPARTMENT_ID
HAVING MIN(SALARY) > (SELECT MIN(SALARY) FROM EMPLOYEE WHERE DEPARTMENT_ID = 100);
...
-- 별칭(ALIAS)
-- 조회된 컬럼의 이름이 너무 길다면 별명을 줘서 대신사용할 수 있다.
-- SELECT절
	-- AS 뒤에 별칭 작성
	-- 한 칸 띄우고 작성
-- FROM절
	-- 테이블 이름 뒤에 한 칸 띄우고 작성

-- 별칭의 특징
-- 테이블에 별칭을 줘서 컬럼을 단순, 명확히 할 수 있다.
-- 조회된 SELECT 문장에서만 유효하다.
-- 테이블 별칭은 길이가 30자까지 가능하나 짧을수록 좋다.
-- 테이블 별칭에는 의미가 있는 것이 좋다.

-- 사원테이블에서 EMPLOYEE_ID를 "사번", FIRST_NAME을 "이름", SALARY를 "급여"로 바꿔서 조회하기
SELECT EMPLOYEE_ID "사번", FIRST_NAME "이름", SALARY "급여"
FROM EMPLOYEES;

SELECT EMPLOYEES.DEPARTMENT_ID, DEPARTMENST.DEPARTMENT_ID
FROM EMPLOYEES, DEPARTMENST;

SELECT E.DEPARTMENT_ID, D.DEPARTMENT_ID
FROM EMPLOYEES E, DEPARTMENST D;
