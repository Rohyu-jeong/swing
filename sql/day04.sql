...
-- 사원들의 이름, 입사일, 입사 후 오늘까지의 개월수를 조회하되, 입사기간이 200개월 이상인 사람만 출력하고
-- 입사 개월수는 소수점 이하 한자리까지만 버림하세요.
SELECT FIRST_NAME, HIRE_DATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HIRE_DATE), 1) "MONTH"
...
SELECT TO_NUMBER(1234567, '9,999,999')
FROM DUAL;
...
SELECT TO_DATE('16', 'DD') FROM DUAL; -- 올해, 지금 월에 해당하는 날짜로 들어감

-- NULL 처리함수
-- NULL 값을 다른 값으로 변경(임시)
...
SELECT FIRST_NAME, COMMISSION_PCT FROM EMPOYEES;
SELECT FIRST_NAME, NVL2(COMMISSION_PCT, '받음', '안받음') FROM EMPOYEES;
...
-- 집계 함수와 일반 컬럼은 일반적인 방법으로는 같이 조회할 수 없다.
